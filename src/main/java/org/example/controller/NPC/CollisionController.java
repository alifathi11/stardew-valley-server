...
//
//// ✨ NEW: JSON collision-only builder
//import org.example.model.utils.TiledCollisionLoader;
//
...
//
//    // Default JSON path fallback if Game.getTiledMap() is null / missing layer
//    private static final String DEFAULT_COLLISION_JSON = "maps/Map1.json";
//
//    private CollisionController(Game game) {
//        this(game, DEFAULT_COLLISION_JSON);
//    }
//
//    // Overload that lets you pass a custom JSON path (e.g., "/maps/Map1.json" for classpath)
//    private CollisionController(Game game, String collisionJsonPath) {
//        this.game = game;
//
//        // Use map from Game if it exists AND has the "Collisions" layer; otherwise build from JSON
//        TiledMap candidate = game.getTiledMap();
//        MapLayer layer = (candidate != null) ? candidate.getLayers().get("Collisions") : null;
//        if (candidate == null || layer == null) {
//            // Build a minimal TiledMap that only contains the "Collisions" layer from Tiled JSON
//            // Set second arg to true if you load from classpath (e.g., "/maps/Map1.json" under resources)
//            candidate = TiledCollisionLoader.buildCollisionOnlyTiledMap(collisionJsonPath, false);
//            layer = candidate.getLayers().get("Collisions");
//            if (layer == null) {
//                throw new IllegalStateException("Collision JSON did not produce a 'Collisions' layer: " + collisionJsonPath);
//            }
//        }
//
//        this.map = candidate;
//
//        MapLayer collisionLayer = layer;
...
//    }
//
//    // Optional: init with explicit JSON path
//    public static void init(Game game, String collisionJsonPath) {
//        if (instance == null) {
//            instance = new CollisionController(game, collisionJsonPath);
//        }
//    }
//
...
//        return !isBlockedByMap(futureRect) && !isBlockedByNPC(futureRect);
//    }
//
